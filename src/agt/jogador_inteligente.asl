...
//retorna o momento que ele esta na roda. se tem duas cartas  quer dizer que é a segunda rodada
codigoVez(RETORNO) :- (
	qtdeCarta(R)
	&
	(
...
//ele pega todas as cartas que o valor é menor que 10 e soma os valores para ter a soma dos pontos envido, contudo nao calcula os pontos bonus por naipes iguais
...
+suavez : true <-
    ...
    .print("Cartas M�o -> ",TODAS_CARTAS_MAO);
    
    ?forcaMao(TODAS_CARTAS_MAO,FORCA_MAO);
    .print("For�a M�o -> ",FORCA_MAO);
...
+!oQueFazerNestaVez(_) : cartaVez(VALOR_VEZ,NIPE_VEZ) <-
	.print("### Tem Carta Vez. � ", VALOR_VEZ, " de ", NIPE_VEZ,"###");
	
	?melhorCartaParaJogar(carta(VALOR_VEZ,NIPE_VEZ), carta(MELHOR_VALOR,MELHOR_NAIPE));
	.print(">>>> Ent�o a melhor carta � ", MELHOR_VALOR, " de ", MELHOR_NAIPE);
...
+truco : true <-
	...
	.print("C�digo Vez -> ",CODIGO_VEZ);
	.print("For�a Mao -> ",FORCA_MAO);
...
+!oQueFazerSeOutroJogadorPedioTruco(_,_,_) : true <-
	.print("N�o aceito truco, O jogo t� muito podre");
	recusar;
.
...
+!oQueFazerSeOutroJogadorPedioRetruco(_,_,_) : true <-
	.print("O jogo t� mais ou menos, mas n�o vou");
	recusar;
.
...
+!oQueFazerSeOutroJogadorPedioVale4(_,_,_) : true <-
	.print("O jogo t� razo�vel, mas n�o vou");
	recusar;
.
